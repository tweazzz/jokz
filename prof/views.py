...
class UniversitiesApi(viewsets.ModelViewSet):
    ...
    def available_university(self, request, *args, **kwargs):
        if self.request.user.is_authenticated:
            if self.request.user.is_superuser:
                ring = Universities.objects.all()
            uni_filter = AvailableUniversitiesFilter(request.GET, queryset=ring)
            serializer = AvailableUniversitiesSerializer(uni_filter.qs, many=True)

            return Response(serializer.data, status=status.HTTP_200_OK)
        else:
            return Response({"detail": "Authentication credentials were not provided."}, status=status.HTTP_401_UNAUTHORIZED)

class ProffessionsApi(viewsets.ModelViewSet):
    queryset = Proffessions.objects.all()
    serializer_class = ProffessionsSerializer
    filter_backends = [DjangoFilterBackend]
    filterset_class = ProffessionsFilter
    permission_classes = [IsSuperAdmin]

class InternshipsApi(viewsets.ModelViewSet):
    queryset = Internships.objects.all()
    serializer_class = InternshipsSerializer
    filter_backends = [DjangoFilterBackend]
    filterset_class = InternshipsFilter
    permission_classes = [IsSuperAdmin]
